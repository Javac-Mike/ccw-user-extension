...
(function (Scratch) {
  ...
  getGLSLFile() {
    return this.getFile(['glsl']);
  }
  getFile(names) {
    const runtime = _Scratch.runtime;
    if (!runtime?.getGandiAssetsFileList) return "";
    let list = [];
    names.forEach((name) => {
      try {
        const items = runtime.getGandiAssetsFileList(name).map((item) => ({
          text: item.fullName,
          value: item.fullName
        }));
